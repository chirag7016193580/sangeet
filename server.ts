...
// Spotify-grade High Performance Cache Layer
const APICache = new Map<string, { data: any, timestamp: number }>();
const CACHE_TTL = 1000 * 60 * 30; // 30 minutes caching for speed
...
function decryptMediaUrl(encryptedBase64: string): string {
  try {
...
function getFullQualityUrl(decryptedUrl: string, has320kbps: boolean): string {
...
async function searchViaSongDetails(query: string) {
  try {
    ...
    if (
      ...
      !autoData.songs ||
      !autoData.songs.data ||
      autoData.songs.data.length === 0
    ) {
      return [];
    }

    const songIds = autoData.songs.data.map((s: any) => s.id).slice(0, 15);
...
async function searchViaGetResults(query: string) {
  try {
...
// Fetch Spotify Playlist or Album details via Embed scraper
async function fetchSpotifyEmbed(type: string, id: string) {
  try {
...
function parseSpotifyTracks(data: any): any[] {
  if (!data) return [];
  const songs: any[] = [];
  ...
  for (const item of items) {
    ...
    songs.push({
      ...
      image: image,
      audioUrl: 'spotify',
      duration: Math.floor((track.duration_ms || 180000) / 1000).toString(),
...
function getStaticFallbackTracks(type: string, id: string): any[] {
  return [
    {
      title: 'Sajni',
...
// API recommendation endpoint for high quality infinite autoplay
app.get('/api/recommend', async (req, res) => {
  try {
    ...
    if (pid && pid !== 'undefined') {
      const liveRes = await axios.get(JIOSAAVN_API, {
        params: {
          __call: 'reco.getreco',
          api_version: '4',
          _format: 'json',
...
// API search endpoint with smart local catalog priority and live API fallbacks
app.get('/api/search', async (req, res) => {
  try {
    const query = (req.query.song || req.query.q || req.query.query) as string;
    if (!query) {
      return res.json([]);
    }
    ...
    let catalogResults: any[] = await prisma.song.findMany({
      where: {
        OR: [
    ...
    catalogResults = catalogResults.map((s: any) => ({ ...s, artist: s.artistName, album: s.albumName }));
...
// API playlists endpoint for fetching category specific playlists
app.get('/api/playlists', async (req, res) => {
  try {
    const category = (req.query.category || req.query.q) as string;
    if (!category) {
      return res.json([]);
    }
    ...
    console.log('\n📚 Fetching Playlists for:', category);
    ...
    let playlists = data.results.map((p: any) => {
      // JioSaavn uses varying fields for popularity/plays in search results
      const plays = parseInt(p.play_count || p.list_count || p.more_info?.follower_count || p.more_info?.play_count || '0', 10);
      return {
        id: p.id,
        ...
        playCount: isNaN(plays) ? 0 : plays,
        type: 'playlist'
      };
    });

    // Sort descending by play count to show most popular ones first
    playlists.sort((a: any, b: any) => b.playCount - a.playCount);
...
// --- PRISMA MUSIC CATALOG COMPREHENSIVE ENDPOINTS ---
...
app.get('/api/catalog/home', async (req, res) => {
  try {
    ...
    const localTopSongs = (await prisma.song.findMany({ orderBy: { popularity: 'desc' }, take: 24 }))
      .map((s: any) => ({...s, artist: s.artistName, album: s.albumName}));
    const localAlbums = await prisma.album.findMany({ take: 10 });
    const localArtists = await prisma.artist.findMany({ take: 10 });
    const localPodcasts = (await prisma.song.findMany({ where: { isPodcast: true }, take: 10 }))
      .map((s: any) => ({...s, artist: s.artistName, album: s.albumName}));
    ...
    const formatImage = (url: string) => (url || '').replace('150x150', '500x500').replace('50x50', '500x500');

    let trending: any[] = [];
    let featuredAlbums: any[] = [];
    let charts: any[] = [];
    let moods: any[] = [];
    let topArtists: any[] = [];
    ...
    if (liveData) {
      if (liveData.new_trending && liveData.new_trending.length > 0) {
        trending = liveData.new_trending.map((s: any) => ({
          id: s.id,
      ...
      if (liveData.new_albums && liveData.new_albums.length > 0) {
        featuredAlbums = liveData.new_albums.map((a: any) => ({
          id: a.id,
      ...
      if (liveData.charts && liveData.charts.length > 0) {
        charts = liveData.charts.map((c: any) => ({
          id: c.id,
      ...
      if (liveData.top_playlists && liveData.top_playlists.length > 0) {
        moods = liveData.top_playlists.map((p: any) => ({
          id: p.id,
          name: decodeHTMLEntities(p.title),
      ...
      if (liveData.artist_recos && liveData.artist_recos.length > 0) {
        topArtists = liveData.artist_recos.map((a: any) => ({
          id: a.id,
          name: decodeHTMLEntities(a.title),
          image: formatImage(a.image),
    ...
    // Powerful Merge Logic: Ensure we never return empty arrays
...
app.get('/api/catalog/artist/:id', async (req, res) => {
  try {
    ...
    let artist: any = await prisma.artist.findUnique({ where: { id: req.params.id } });
    ...
    if (!artist) {
      ...
      if (d.topSongs && d.topSongs.length > 0) {
        popularTracks = d.topSongs.map((t: any) => ({
          id: t.id,
      ...
      // ENRICH ARTIST TRACKS: Fetch 50 more tracks via Search for "Bohat Sare Music"
      try {
    ...
    // Prisma Fallback
    popularTracks = (await prisma.song.findMany({ where: { artistId: artist.id }, orderBy: { plays: 'desc' }, take: 10 })).map((s: any) => ({...s, artist: s.artistName, album: s.albumName}));
...
app.get('/api/catalog/album/:id', async (req, res) => {
  try {
    ...
    if (!album) {
      ...
      const tracks = (liveAlbum.list || []).map((t: any) => ({
        id: t.id,
        title: decodeHTMLEntities(t.title),
    ...
    let tracks = (await prisma.song.findMany({ where: { albumId: album.id } })).map((s: any) => ({...s, artist: s.artistName, album: s.albumName}));
    ...
    if (tracks.length > 0 && tracks[0].isSpotify) {
       ...
       if (liveTracks.length > 0) {
          tracks = liveTracks.map((t: any) => ({...t, image: album.image, album: album.name}));
       }
    }
...
app.get('/api/catalog/playlist/:id', async (req, res) => {
  try {
    ...
    if (!playlist) {
      ...
      const tracks = (liveList.list || []).map((t: any) => ({
        id: t.id,
        title: decodeHTMLEntities(t.title),
        artist: decodeHTMLEntities(t.more_info?.artistMap?.primary_artists?.[0]?.name || t.subtitle || 'Unknown'),
        image: (t.image || '').replace('150x150', '500x500').replace('50x50', '500x500'),
    ...
    const tracks = playlistSongs.map((ps: any) => ({ ...ps.song, artist: ps.song.artistName, album: ps.song.albumName }));

    const similarPlaylists = await prisma.playlist.findMany({
      where: { id: { not: playlist.id }, OR: [{ type: playlist.type }, { genre: playlist.genre || undefined }] },
      take: 6
    });
...
app.get('/api/catalog/discover', async (req, res) => {
  try {
    ...
    if (type === 'album') {
      ...
      items = (await prisma.song.findMany({ where: { isPodcast: true }, orderBy: { popularity: 'desc' }, skip, take: limit })).map((s: any) => ({...s, artist: s.artistName, album: s.albumName}));
      total = await prisma.song.count({ where: { isPodcast: true } });
    } else {
      const whereClause = genre ? { genre, isPodcast: false } : { isPodcast: false };
      items = (await prisma.song.findMany({ where: whereClause, orderBy: { popularity: 'desc' }, skip, take: limit })).map((s: any) => ({...s, artist: s.artistName, album: s.albumName}));
  ...
  try {
    ...
    if (country && country !== 'Global') {
      const countryPlaylist = await prisma.playlist.findFirst({
        where: { type: 'chart', country: country }
      });
      if (countryPlaylist) {
        const ps = await prisma.playlistSong.findMany({ where: { playlistId: countryPlaylist.id }, include: { song: true }, take: limit });
        return res.json({ title: `${country} Top Chart`, tracks: ps.map((p: any) => ({...p.song, artist: p.song.artistName, album: p.song.albumName})) });
      }
    }

    const whereClause: any = { isPodcast: false };
...
app.get('/api/recommendations', async (req, res) => {
  try {
    ...
    if (userId) {
      ...
      if (history.length > 0) {
        const genres = Array.from(new Set(history.map((h: any) => h.song.genre).filter(Boolean))) as string[];
        const artists = Array.from(new Set(history.map((h: any) => h.song.artistName).filter(Boolean))) as string[];
        const historySongIds = history.map((h: any) => h.songId);
    ...
    res.json({ 
      ...
      tracks: recommendedSongs.map((s: any) => ({...s, artist: s.artistName, album: s.albumName})) 
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
