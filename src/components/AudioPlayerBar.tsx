      ...
      <div className="fixed bottom-20 md:bottom-0 left-0 right-0 z-40 px-4 md:px-0 select-none">
        <div className="max-w-7xl mx-auto bg-black/40 backdrop-blur-2xl border border-white/5 md:border-0 md:border-t md:border-white/5 md:bg-black/20 p-3 md:p-6 rounded-2xl md:rounded-none flex items-center justify-between gap-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1 md:flex-initial md:w-80 cursor-pointer" onClick={() => setIsFullscreen(true)}>
            ...
            <div className="flex md:hidden items-center gap-1 shrink-0 ml-auto mr-1">
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  toggleLike(currentTrack);
                }}
                className="p-2 text-neutral-400 hover:text-[#bc8aff] transition-all"
              >
                <Heart className={`w-5 h-5 transition-transform duration-300 ${isLiked(currentTrack) ? 'text-[#bc8aff] fill-[#bc8aff] scale-110' : ''}`} />
              </button>
              <button 
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <button 
              ...
              className="hidden md:block p-2 text-neutral-400 hover:text-[#bc8aff] transition-all shrink-0"
            >
              <Heart 
          ...
          <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-white/10 md:hidden overflow-hidden rounded-b-2xl">
            <div 
              className="h-full bg-[#bc8aff] transition-all duration-300 ease-linear"
