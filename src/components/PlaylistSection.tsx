    ...
    <section className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          {!showAll && (
            <div className="hidden md:flex items-center gap-2 mr-2">
              <button onClick={() => scroll('left')} className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-white transition-colors border border-white/5">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={() => scroll('right')} className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-white transition-colors border border-white/5">
      ...
      <div 
        ...
        className={showAll 
          ...
          : "flex gap-4 overflow-x-auto hide-scrollbar -mx-4 px-4 md:mx-0 md:px-0 scroll-smooth"}
      >
        {(showAll ? playlists : playlists.slice(0, 10)).map((playlist: any) => (
          <div 
            key={playlist.id} 
            onClick={() => setViewDetails({ type: 'playlist', id: playlist.id })}
            className={`${showAll ? 'w-full' : 'flex-none w-36 md:w-44'} group cursor-pointer`}
          >
