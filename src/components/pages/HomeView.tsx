...
export const HomeView: React.FC<HomeViewProps> = ({ setViewDetails, setActiveTab }) => {
  ...
  const featuredArtistsList = feed?.topArtists || CURATED_ARTISTS.slice(0, 8);
  ...
  return (
    <div className="space-y-10 pb-32 animate-fade-in select-none">
      ...
      <section className="space-y-4">
        ...
        {loadingFeed ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 animate-pulse">
            {[1, 2, 3, 4, 5, 6].map((i) => (
            ...
            {(history && history.length > 0 ? history : quickPickList).slice(0, 6).map((item: any) => {
              ...
              return (
                <div 
                  ...
                  className="flex items-center gap-4 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 rounded-2xl overflow-hidden group cursor-pointer transition-all duration-300 relative pr-4"
                >
                  <div className="w-16 h-16 shrink-0 relative bg-neutral-950">
                    <img 
                      src={item.image || null} 
      ...
      <section className="space-y-4">
        <div className="flex items-end justify-between mb-5">
          ...
          <span className="font-mono text-[9px] text-white/40 uppercase tracking-[0.2em]">Artists</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {featuredArtistsList.slice(0, 8).map((artist: any) => (
            <div 
              key={artist.id}
