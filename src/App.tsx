...
function Dashboard() {
  ...
  // Sync back button with virtual views
  useEffect(() => {
  ...
  }, [viewDetails, activeTab]);

  const navigateDetails = (details: any) => {
    if (details) {
  ...
  const navigateTab = (tab: 'home' | 'search' | 'library' | 'settings') => {
    if (tab !== activeTab) {
  ...
  const renderActiveView = () => {
    if (viewDetails) {
      return (
        <DetailView 
          ...
          setViewDetails={navigateDetails}
          onBack={() => {
            window.history.back();
            // Fallback just in case history is empty
            setTimeout(() => setViewDetails(null), 50);
          }} 
        />
      );
    }
    ...
    switch (activeTab) {
      ...
      case 'search':
        return <SearchView />;
      case 'library':
        return <LibraryView setViewDetails={navigateDetails} />;
      case 'settings':
        return <SettingsView />;
    }
  };
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-[#08080a]/80 backdrop-blur-xl px-4 md:px-6 py-3 md:py-4 flex items-center justify-between border-b border-white/5">
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-6">
            <div 
              onClick={() => { navigateTab('search'); navigateDetails(null); }}
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div 
              onClick={() => { navigateTab('settings'); navigateDetails(null); }}
